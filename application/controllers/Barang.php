	...
	public function tambah()
	{
		...
		$this->form_validation->set_rules('ukuran_barang', 'Ukuran Barang', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Ukuran Barang Tidak Boleh Kosong.</div>'
					)
			);
		$this->form_validation->set_rules('stok_barang', 'Stok Barang', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Stok Barang Tidak Boleh Kosong.</div>'
					)
			);
		$this->form_validation->set_rules('id_jenis_barang', 'Jenis Barang', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Jenis Barang Tidak Boleh Kosong.</div>'
					)
			);
		$this->form_validation->set_rules('id_desain', 'Desain Barang', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Desain Barang Tidak Boleh Kosong.</div>'
					)
			);
		$this->form_validation->set_rules('harga_jual_barang', 'Harga', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Harga Jual Barang Tidak Boleh Kosong.</div>'
					)
			);
		$this->form_validation->set_rules('keterangan_barang', 'Keterangan', 'required|trim',
				array(
					'required' => '<div class="alert alert-danger">Gagal! Keterangan Tidak Boleh Kosong.</div>'
					)
			);
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($this->uri->segment(3) == "penjualan" || $this->uri->segment(3) == "Penjualan") {
